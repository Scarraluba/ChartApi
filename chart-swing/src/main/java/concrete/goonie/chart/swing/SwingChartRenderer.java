...
public final class SwingChartRenderer {
    ...
    public void draw(Graphics2D g, RenderList list) {
        ...
        Deque<Shape> clipStack = new ArrayDeque<>();
        for (RenderCommand cmd : list.items()) {
            if (cmd instanceof RenderCommand.ClipPush c) { clipStack.push(g.getClip()); g.setClip(new Rectangle(Math.round(c.left), Math.round(c.top), Math.max(0, Math.round(c.right - c.left)), Math.max(0, Math.round(c.bottom - c.top)))); }
            else if (cmd instanceof RenderCommand.ClipPop) { g.setClip(clipStack.isEmpty() ? null : clipStack.pop()); }
            else if (cmd instanceof RenderCommand.Line c) drawLine(g,c); else if (cmd instanceof RenderCommand.RoundedRect c) drawRoundedRect(g,c); else if (cmd instanceof RenderCommand.Rect c) drawRect(g,c); else if (cmd instanceof RenderCommand.Circle c) drawCircle(g,c); else if (cmd instanceof RenderCommand.Text c) drawText(g,c);
        }
        if (!clipStack.isEmpty()) { g.setClip(clipStack.peekLast()); }
    }
